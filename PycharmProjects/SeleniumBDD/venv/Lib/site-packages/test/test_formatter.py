...
class FormatterTests(object):
    ...
    def _feature(self, keyword='k\xe9yword', name='name', tags=['spam', 'ham'],
            location='location', description=['description'], scenarios=[],
            background=None):
        line = self.line
    ...
    def _scenario(self, keyword='k\xe9yword', name='name', tags=[], steps=[]):
        line = self.line
        tags = [Tag(name, line) for name in tags]
        return Scenario('<string>', line, keyword, name, tags=tags, steps=steps)

    def _step(self, keyword='k\xe9yword', step_type='given', name='name', text=None, table=None):
        line = self.line
